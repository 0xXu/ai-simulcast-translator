...
- 语言范围：源语言自动检测或手动指定，目标语言从常用语言中选择
- 设计日期：2026-06-06
- 文档状态：已确认设计
...
3. 使用 MiMo 完成多语言翻译、上下文理解和历史字幕修订。
...
MVP 中 MiMo 返回最近可修订区的完整结构化字幕快照。客户端使用递增
`requestId` 丢弃过期响应，并按字幕顺序原位替换；不要求模型生成细粒度
`upsert`/`replace` 操作。
...
- 默认使用多语言模型 `small`；性能较好的设备可切换 `medium`。
...
PR 08 直接展示最新原始窗口。MVP 不单独实现 `TranscriptStabilizer`；
最近约 20 秒的重叠原文由 `mimo-v2.5` 在生成字幕快照时统一整理和分段。
...
存在新的 Whisper 原始窗口，且距离上次调度约 1.2 至 1.5 秒时，可以创建
字幕协调请求。同时应用以下限制：
...
1. 固定系统指令。
2. 最近约 20 秒的 faster-whisper 原始识别窗口。
3. 最近 5 句或 20 秒内当前已展示的双语字幕。
4. 递增的 `requestId`。

### 12.1 模型职责

- 整理重叠 Whisper 窗口并形成可读的源语言句子。
- 按会话设置翻译成自然、简洁的目标语言字幕。
- 保留数字、单位、代码、产品名和专有名词。
- 根据后续上下文修正可修订区中的错误。
- 仅在确有语义依据时修改历史字幕。
- 不修改输入中标记为 `locked` 的内容。
- 返回完整的可修订字幕快照，输出严格 JSON，不输出解释或 Markdown。
...
模型只能返回最近可修订窗口。客户端不允许快照修改已经锁定的字幕。

## 13. 版本化修订机制

实时请求可能乱序返回。协调器记录最后应用的 `requestId`，小于或等于该值的
响应直接丢弃。通过 Schema 校验且更新的响应，只能替换仍处于 `live` 或
`revisable` 状态的最近字幕；锁定字幕保持不变。
...
1. macOS 系统播放受支持语言内容时，悬浮窗可持续显示所选目标语言翻译。
2. 应用无需上传原始音频即可完成本地 ASR。
3. MiMo 负责跨语言翻译和语义修订；同语种模式直接显示原文。
...
- 验证 `mimo-v2.5` 的结构化字幕快照格式和平均延迟。
- 用固定重叠原文验证去重、分段、翻译和后文纠错。

### 阶段二：核心流水线

- 建立 Electron、React 和 Python Worker。
- 完成音频、ASR、MiMo 字幕协调和 IPC 链路。
- 使用 `requestId` 和字幕锁定规则应用快照。
...
MiMo 不应为了语言润色反复改写历史。提示词和响应校验应要求只有语义错误
才改变已有字幕，并将修改限制在最近 5 句或 20 秒内。
...
4. **结构化字幕快照**：模型返回可校验的有限窗口快照，客户端拒绝旧响应和
   越界修改。
5. **原文整理与语义修订协同**：一次模型请求同时处理 ASR 重叠文本、翻译消歧
   和最近字幕纠错。
...
- 当官方提供持续音频流接口时，可评估接入 MiMo-V2.5-ASR 或其他云端流式 ASR。
- 中文 TTS 同传语音。
- 用户自定义术语库和会前资料导入。
- 会后双语稿、摘要和知识点。
- 会话中动态切换语言和句内混合语言分段。
